...
\centering
\footnotesize
\caption{False authority (FAR) and utility for the main stores, five seeds each unless noted ($\pm$ = 95\% interval over seeds). The full ablation with revocation survival, generalization, legitimate rejections and invariant counts is Table~\ref{tab:C1}.}
\label{tab:1}
...
\caption{False authority for four stores across four model families and two domains (the full ablation with utility and invariant counts is Table~\ref{tab:C1} in the appendix).}
\label{fig:1}
\end{figure}
...
\caption{False authority against decay aggressiveness for the three type-blind policies and THSM, gpt-4o-mini, 15 seeds per type-blind point.}
\label{fig:2}
\end{figure}
...
\caption{Utility--authority frontier for the same sweep. Type-blind curves drift down and left as decay tightens; THSM moves only left.}
\label{fig:3}
\end{figure}
...
\caption{Prohibition compliance by depth since the DENY, three models.}
\label{fig:4}
\end{figure}
