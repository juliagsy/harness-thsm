...
Harness state is a set of entries e = $\langle$id, $\tau$, c, $\pi$, $\ell$, T, a$\rangle$ with type $\tau$ $\in$ \{EPI, SEM, PROC, DEON\}, payload c, provenance $\pi$ (source entry ids and writer identity), integrity label $\ell$, bi-temporal validity T (created, valid-from, valid-to, expired; supersede and invalidate, never delete), and activation a defined only for SEM and PROC.
